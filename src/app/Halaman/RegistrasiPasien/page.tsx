...
export default function RegistrasiPasien() {
    ...
    return (
        <motion.div
            ...
            className="min-h-screen bg-gradient-to-tr from-white via-blue-100 to-blue-200 p-6 mb-8 flex flex-col justify-center" // Removed card classes
            //min-h-screen bg-gradient-to-tr from-white via-blue-100 to-blue-200 flex items-center justify-center p-4
        >
            <Toaster position="top-right" />
            <Modal isOpen={stateShowModal} onClose={() => setStateShowModal(false)} setStateForm={setStateForm} />
            <h1 className="text-2xl font-bold mb-1 text-center text-black-500">RSUD I.A. MOEIS SAMARINDA</h1> {/* Changed text color to yellow */}
            <h5 className='text-black-600 mb-5 text-center'>REGISTRASI MEDICAL CHECK UP</h5> {/* Changed text color to yellow */}
            <form className="w-full space-y-4 bg-card p-8 rounded-2xl shadow-lg space-y-4 border border-border">
                {/* NIK */}
                <div>
                    <label className='text-black-500'><h5>NIK</h5></label>
                    <input
                        type="number"
                        value={stateForm?.nik}
                ...
                {/* NOPE */}
                <div>
                    <label className="block text-sm font-medium text-black-400">NO WHATSAPP</label>
                    <input
                        type="number"
                ...
                {/* Berat Badan */}
                <div>
                    <label className="block text-sm font-medium text-black-400">Berat Badan (kg)</label>
                    <input
                        type="number"
                        value={stateForm?.bb}
                        onChange={(e) => setStateForm((prev) => ({
                ...
                {/* Tinggi Badan */}
                <div>
                    <label className="block text-sm font-medium text-black-400">Tinggi Badan (cm)</label>
                    <input
                        type="number"
                        value={stateForm?.tb}
                        onChange={(e) => setStateForm((prev) => ({
                ...
                {/* Golongan Darah */}
                <div>
                    <label className="block text-sm font-medium text-black-400">Golongan Darah</label>
                    <Select
                        placeholder="--Pilih--"
                        className="text-black-400"
                        options={GolDarah}
                        value={GolDarah.find(opt => opt.value === stateForm.gol_darah) || null}
                        onChange={(e) => setStateForm((prev) => ({
                            ...prev,
                            gol_darah: e?.value || ''
                        }))}
                        styles={{
                            ...
                            option: (base, state) => ({
                                ...base,
                                backgroundColor: state.isSelected ? 'pink' : 'inherit',  // Atur background ketika opsi dipilih
                                color: state.isSelected ? 'black' : 'black',  // Atur warna teks ketika dipilih
                                '&:hover': {
                                    backgroundColor: 'blue', // Background saat hover
                ...
                {/* Keperluan */}
                <div>
                    <label className="block text-sm font-medium text-black-400">Keperluan</label>
                    <textarea
